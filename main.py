...
class Ball:
    ...
    def update(self):
        ...
        dirToCenter = Vector2(self.position.x - width/2,
                              self.position.y - height/2)
        if self.isCollide():
            pygame.mixer.Sound.play(sound)
            self.radius += 1
            self.position = Vector2(self.prevPos.x, self.prevPos.y)
            v = math.sqrt(self.velocity.x * self.velocity.x +
            ...
            self.velocity = Vector2(-v * math.cos(newAngle),
                                    v * math.sin(newAngle))
